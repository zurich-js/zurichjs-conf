...
async function resolveSponsorCurrencyConversion(
  ...
  sponsorCurrency: SponsorshipCurrency | null;
  sponsorAmount: number | null;
  sponsorToChfRate: number | null;
  sponsorRateDate: string | null;
  sponsorRateSource: 'frankfurter' | 'bank' | 'manual' | 'other' | null;
  payableAmountChf: number | null;
  payableRounding: 'nearest_1_chf' | null;
}> {
  if (!data.sponsorCurrency || data.sponsorAmount === undefined) {
    return {
      sponsorCurrency: null,
      sponsorAmount: null,
      sponsorToChfRate: null,
      sponsorRateDate: null,
      sponsorRateSource: null,
      payableAmountChf: null,
      payableRounding: null,
    };
  }

  if (data.sponsorAmount <= 0) {
    throw new Error('Sponsor amount must be positive');
  }

  if (data.sponsorCurrency === 'CHF') {
    return {
      sponsorCurrency: 'CHF',
      sponsorAmount: data.sponsorAmount,
      sponsorToChfRate: 1,
      sponsorRateDate: data.sponsorRateDate ?? new Date().toISOString().split('T')[0],
      sponsorRateSource: data.sponsorRateSource ?? 'manual',
      payableAmountChf: convertCurrencyMinorToChf(data.sponsorAmount, 'CHF', {}),
      payableRounding: 'nearest_1_chf',
    };
  }

  if (data.sponsorToChfRate !== undefined) {
    if (data.sponsorToChfRate <= 0) {
      throw new Error('Sponsor to CHF rate must be positive');
    }
    return {
      sponsorCurrency: data.sponsorCurrency,
      sponsorAmount: data.sponsorAmount,
      sponsorToChfRate: data.sponsorToChfRate,
      sponsorRateDate: data.sponsorRateDate ?? new Date().toISOString().split('T')[0],
      sponsorRateSource: data.sponsorRateSource ?? 'manual',
  ...
  const rates = await getExchangeRates('CHF', [data.sponsorCurrency as SponsorshipDisplayCurrency]);
  const chfToSponsorRate = rates.rates[data.sponsorCurrency];
  if (!chfToSponsorRate) {
    throw new Error(`Missing CHF to ${data.sponsorCurrency} exchange rate`);
  }

  return {
    sponsorCurrency: data.sponsorCurrency,
    sponsorAmount: data.sponsorAmount,
    sponsorToChfRate: 1 / chfToSponsorRate,
    sponsorRateDate: rates.date,
...
export async function createInvoice(
  ...
  const sponsorConversion = await resolveSponsorCurrencyConversion(data);
  ...
  const invoiceInsert = {
    ...
    sponsor_currency: sponsorConversion.sponsorCurrency,
    sponsor_amount: sponsorConversion.sponsorAmount,
    sponsor_to_chf_rate: sponsorConversion.sponsorToChfRate,
    sponsor_rate_date: sponsorConversion.sponsorRateDate,
    sponsor_rate_source: sponsorConversion.sponsorRateSource,
    payable_amount_chf: sponsorConversion.payableAmountChf,
    payable_rounding: sponsorConversion.payableRounding,
  };

  const { data: invoice, error } = await supabase
    .from('sponsorship_invoices')
    .insert(invoiceInsert as unknown as never)
    .select()
    .single();
...
export async function updateSponsorInvoiceConversion(
  invoiceId: string,
  data: UpdateSponsorInvoiceConversionRequest,
): Promise<SponsorshipInvoice> {
  const supabase = createServiceRoleClient();
  const conversion = await resolveSponsorCurrencyConversion(data);

  const updateData = {
    sponsor_currency: conversion.sponsorCurrency,
    sponsor_amount: conversion.sponsorAmount,
    sponsor_to_chf_rate: conversion.sponsorToChfRate,
    sponsor_rate_date: conversion.sponsorRateDate,
    sponsor_rate_source: conversion.sponsorRateSource,
    payable_amount_chf: conversion.payableAmountChf,
    payable_rounding: conversion.payableRounding,
  };
  ...
  if (error) {
    console.error('Error updating sponsor invoice conversion:', error);
    throw new Error(`Failed to update sponsor invoice conversion: ${error.message}`);
  }
