  ...
  if (submitSuccess) {
    const successContent = (
      <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
        ...
        <button onClick={resetForm} className="px-6 py-2.5 bg-green-600 text-brand-white rounded-lg text-sm font-medium hover:bg-green-700 transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {types.map((t) => (
          <button key={t.id} type="button" onClick={() => setPaymentType(t.id)}
            className={`p-4 rounded-xl border-2 text-left transition-all cursor-pointer ${paymentType === t.id ? 'border-brand-primary bg-brand-primary/10' : 'border-brand-gray-lightest hover:border-gray-300'}`}>
            <div className="flex items-center space-x-3">
              ...
              <div><p className="font-bold text-black">{t.label}</p><p className="text-xs text-brand-gray-dark">{t.desc}</p></div>
            </div>
          </button>
        ))}
      </div>
    ...
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input type="text" value={stripePaymentId} onChange={(e) => setStripePaymentId(e.target.value)} placeholder="pi_xxx, ch_xxx, or cs_xxx"
          className="flex-1 border border-gray-300 rounded-lg px-4 py-2.5 text-black placeholder-brand-gray-medium focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-transparent" />
      ...
      {stripePayment && (
        <div className="mt-3 p-3 bg-white rounded-lg border border-blue-200">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div><span className="text-brand-gray-dark">Amount:</span><span className="ml-1 font-bold text-black">{(stripePayment.amount / 100).toFixed(2)} {stripePayment.currency}</span></div>
            <div><span className="text-brand-gray-dark">Status:</span><span className={`ml-1 font-bold ${stripePayment.status === 'succeeded' || stripePayment.status === 'paid' ? 'text-green-600' : 'text-yellow-600'}`}>{stripePayment.status}</span></div>
            {stripePayment.customerName && <div className="col-span-2"><span className="text-brand-gray-dark">Customer:</span><span className="ml-1 text-black">{stripePayment.customerName}</span></div>}
            {stripePayment.customerEmail && <div className="col-span-2"><span className="text-brand-gray-dark">Email:</span><span className="ml-1 text-black">{stripePayment.customerEmail}</span></div>}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-brand-gray-dark mb-1">Amount Received *</label>
          <input type="number" step="0.01" min="0" required value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="150.00"
            className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-black placeholder-brand-gray-medium focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-transparent" />
        </div>
        <div>
          <label className="block text-xs font-medium text-brand-gray-dark mb-1">Currency *</label>
        ...
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-brand-gray-dark mb-1">Reference / Notes (optional)</label>
    ...
    <div className="border-t border-brand-gray-lightest pt-6">
      <label className="flex items-center space-x-3 cursor-pointer">
        ...
        <div>
          ...
          <p className="text-xs text-brand-gray-dark">Send ticket with QR code and details to the attendee</p>
        </div>
      </label>
    </div>
  );
