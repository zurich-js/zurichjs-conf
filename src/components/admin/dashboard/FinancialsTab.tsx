...
export function FinancialsTab() {
  ...
  if (!financials) return <div className="bg-white rounded-xl shadow-lg p-12 text-center border border-brand-gray-lightest"><p className="text-base font-medium text-black">Failed to load financial data</p></div>;
    ...
    <div className="space-y-4">
      ...
      {multiCurrency && (
        <div className="bg-black rounded-xl p-4 sm:p-6 border border-gray-800">
          <h3 className="text-xs sm:text-sm font-bold text-brand-gray-medium uppercase tracking-wide mb-3">Grand Total (all currencies combined)</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <p className="text-xs text-brand-gray-medium uppercase font-medium">Gross</p>
              <p className="text-2xl sm:text-3xl font-bold text-white">{formatCHF(grand.totalGross)}</p>
            </div>
            <div>
              <p className="text-xs text-brand-gray-medium uppercase font-medium">Fees</p>
              <p className="text-2xl sm:text-3xl font-bold text-purple-400">-{formatCHF(grand.totalFees)}</p>
            </div>
            <div>
              <p className="text-xs text-brand-gray-medium uppercase font-medium">Net</p>
        ...
        return (
          <div key={cur}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {cards.map((c) => (
                <div key={c.title} className="bg-white shadow-lg rounded-xl p-4 sm:p-6 border border-brand-gray-lightest">
                  ...
                  <p className="mt-2 text-xs sm:text-sm text-brand-gray-dark font-medium">{c.subtitle}</p>
                </div>
              ))}
            </div>
          </div>
...
function RevenueBreakdown({ summary, sponsorshipSummary, workshopSummary }: { summary: FinancialData['summary']; sponsorshipSummary?: FinancialData['sponsorshipSummary']; workshopSummary?: FinancialData['workshopSummary'] }) {
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        <h3 className="text-lg sm:text-xl font-bold text-black">Revenue Breakdown</h3>
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">How the combined gross revenue is composed</p>
      </div>
...
function BreakdownRow({ icon: Icon, label, value, detail, color }: { icon: typeof Ticket; label: string; value: string; detail?: string; color: string }) {
  return (
    <div className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg border border-brand-gray-lightest">
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        ...
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">Individual sales, B2B sales, and complimentary tickets</p>
      </div>
      <div className="p-4 sm:p-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-gradient-to-br from-gray-50 to-text-brand-gray-lightest rounded-lg p-5 border border-brand-gray-lightest">
            ...
            <p className="text-sm text-brand-gray-dark mt-1">Free tickets (no payment)</p>
          </div>
        </div>
...
function ChannelCard({ title, icon: Icon, color, amount, count, fees, details }: { title: string; icon: typeof Ticket; color: string; amount: number; count: number; fees: number; details: { label: string; count: number; amount: number }[] }) {
  ...
  return (
    <div className={`bg-gradient-to-br ${c.bg} rounded-lg p-5 ${c.border} border`}>
      ...
      <p className="text-sm text-brand-gray-dark mt-1">{count.toLocaleString('de-CH')} tickets</p>
        ...
        <div className={`mt-4 pt-3 border-t ${c.divider} space-y-2 text-sm`}>
          {details.map((d) => (
            <div key={d.label} className="flex justify-between">
              <span className="text-brand-gray-dark">{d.label}</span>
...
function B2BPipeline({ summary }: { summary: NonNullable<FinancialData['b2bSummary']> }) {
  return (
    <div className="mt-6 pt-6 border-t border-brand-gray-lightest">
      <h4 className="font-semibold text-black mb-3">B2B Invoice Pipeline</h4>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div className="p-3 bg-green-50 rounded-lg border border-green-200">
          <p className="text-brand-gray-dark text-xs font-medium uppercase">Paid</p>
        ...
        <div className="p-3 bg-amber-50 rounded-lg border border-amber-200">
          <p className="text-brand-gray-dark text-xs font-medium uppercase">Pending</p>
        ...
        <div className="p-3 bg-gray-50 rounded-lg border border-brand-gray-lightest">
          <p className="text-brand-gray-dark text-xs font-medium uppercase">Draft</p>
          <p className="font-bold text-brand-gray-dark text-lg">{summary.draftInvoices}</p>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg border border-brand-gray-lightest">
          <p className="text-brand-gray-dark text-xs font-medium uppercase">Total</p>
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        ...
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">{summary.soldSeats} sold {summary.soldSeats === 1 ? 'seat' : 'seats'}</p>
      </div>
      <div className="p-4 sm:p-6">
          ...
          {currencies.map((currency) => (
            <div key={currency} className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
              ...
              {(summary.stripeFeesByCurrency[currency] || 0) > 0 && (
                <p className="text-xs text-brand-gray-dark mt-1">Stripe fees: {formatCHF(summary.stripeFeesByCurrency[currency])} {currency}</p>
              )}
              {(summary.discountsByCurrency[currency] || 0) > 0 && (
                <p className="text-xs text-brand-gray-dark mt-1">Discounts: {formatCHF(summary.discountsByCurrency[currency])} {currency}</p>
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        ...
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">{summary.totalDeals} deals total ({summary.paidDeals} paid, {summary.pendingDeals} pending)</p>
      </div>
      <div className="p-4 sm:p-6">
        {/* Revenue by currency */}
          ...
          <div>
            ...
            <div className="space-y-2">
              {Object.entries(summary.byTier).map(([tier, data]) => (
                <div key={tier} className="flex flex-col sm:flex-row justify-between sm:items-center gap-1 p-3 bg-gray-50 rounded-lg border border-brand-gray-lightest">
                  <div className="flex items-center gap-2">
                    <Handshake className="w-4 h-4 text-brand-gray-medium" />
                  ...
                  <div className="flex gap-3 text-sm">
                    ...
                    {data.revenueCHF === 0 && data.revenueEUR === 0 && <span className="text-brand-gray-medium text-xs">No paid revenue</span>}
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        ...
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">Daily ticket sales and cumulative total</p>
      </div>
      <div className="p-4 sm:p-6">
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        ...
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">Breakdown by ticket type and workshop sales</p>
      </div>
      <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
        {Object.entries(byCategory).map(([category, data]) => (
          <div key={category} className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 p-3 sm:p-4 bg-gray-50 rounded-lg border border-brand-gray-lightest">
            <span className="font-bold text-black capitalize text-sm sm:text-base">{category}</span>
            <div className="text-left sm:text-right"><span className="text-black font-bold text-base sm:text-lg">{formatCHF(data.revenue)} CHF</span><span className="text-brand-gray-dark text-xs sm:text-sm ml-2 sm:ml-3 font-medium">({data.count} {data.count === 1 ? 'ticket' : 'tickets'})</span></div>
          </div>
        ))}
        {workshopSummary && workshopSummary.soldSeats > 0 && (
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 p-3 sm:p-4 bg-emerald-50 rounded-lg border border-emerald-200">
            ...
            <div className="text-left sm:text-right">
              ...
              <span className="text-brand-gray-dark text-xs sm:text-sm ml-2 sm:ml-3 font-medium">
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-brand-gray-lightest">
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-brand-gray-lightest bg-gradient-to-r from-gray-50 to-white">
        ...
        <p className="text-xs sm:text-sm text-brand-gray-dark mt-1">Breakdown by pricing period</p>
      </div>
      <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
        {Object.entries(byStage).map(([stage, data]) => (
          <div key={stage} className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 p-3 sm:p-4 bg-gray-50 rounded-lg border border-brand-gray-lightest">
