  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex flex-wrap rounded-lg border border-gray-200 bg-white p-1" role="group" aria-label="Filter attendees">
          {FILTERS.map((item) => (
            <button
              key={item.id}
              type="button"
        ...
        <div className="relative max-w-xs flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-200 bg-white py-2 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary"
          />
        </div>

        <div className="flex items-center gap-3 sm:ml-auto">
          <span className="text-sm text-gray-500">
            {filteredAttendees.length} of {data.attendees.length}
          </span>
          <button
            type="button"
            onClick={() => refetch()}
            disabled={isFetching}
            className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-default disabled:opacity-60"
          >
      ...
      <p className="text-xs text-gray-500">
        People are matched across sources by email, so a plus one whose VIP ticket has already been issued
        (or a speaker who also bought a VIP ticket) counts once. Speaker RSVPs and plus ones come from the
        Speakers → Logistics tab, where additional after-party guests are managed too. Capacity is {data.stats.capacity};
        going over only shows a warning.
      </p>
    </div>
  );
}
