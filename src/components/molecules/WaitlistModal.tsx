/**
 * Waitlist Modal
 * Config-driven modal for anything that can sell out. Explains what the item is
 * and, when sold out, collects emails so users can be notified when it returns.
 * Wrapped by `TicketWaitlistModal` (ticket types) and `WorkshopWaitlistModal`.
 */
...
/**
 * Static content + submission target for a single waitlist.
 */
export interface WaitlistModalConfig {
  /** Stable slug used to build the email input's id */
  id: string;
  /** API endpoint the subscribe form POSTs to */
  endpoint: string;
  /** Extra fields merged into the POST body alongside `email` */
  payload?: Record<string, string>;
  /** Header icon (lucide) */
  icon: LucideIcon;
  /** Modal title */
  title: string;
  /** Intro paragraph explaining what this is */
  description: string;
...
export interface WaitlistModalProps {
  ...
  onClose: () => void;
  /** Whether the item is currently sold out */
  isSoldOut: boolean;
  /** Content + submission target for this waitlist */
  config: WaitlistModalConfig;
}

/**
 * WaitlistModal component
 * Explains an item and collects emails for notifications when sold out
 */
export const WaitlistModal: React.FC<WaitlistModalProps> = ({
  ...
  const inputId = `${config.id}-notify-email`;
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(config.endpoint, {
        ...
        body: JSON.stringify({ email, ...config.payload }),
      });
